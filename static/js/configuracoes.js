...
(async () => {
  ...
  Layout.set(`
    ...
    <div class="card" style="max-width:760px"><div class="card__body">
      ...
      <p class="text-muted" style="margin:0 0 12px;font-size:13px">
        Configure a emiss&atilde;o de NFS-e (nota de servi&ccedil;o municipal).
        O c&oacute;digo IBGE do munic&iacute;pio &eacute; exigido pela maioria dos provedores.
      </p>
      <div class="form-grid" id="cfg-form-nfse">
        ...
        <div class="field col-2"><label>Provedor NFS-e</label>
          <select name="nfse_provedor">
            ${["","focus","plugnotas","nfeio","enotas"].map((p)=>{
              const nm={"":"&mdash; selecione &mdash;",focus:"Focus NFe",plugnotas:"PlugNotas",nfeio:"NFe.io",enotas:"eNotas"};
              return `<option value="${p}" ${(c.nfse_provedor||"")===p?"selected":""}>${nm[p]}</option>`;
            }).join("")}
          </select></div>
        <div class="field col-2"><label>Token / chave da API</label>
          <input name="nfse_token" value="${c.nfse_token||""}" placeholder="Cole aqui o token do provedor" autocomplete="off"></div>
        ...
        <div class="field col-2"><label>Inscri&ccedil;&atilde;o Municipal</label>
          <input name="empresa_inscricao_municipal" value="${c.empresa_inscricao_municipal||""}" placeholder="N&uacute;mero da inscri&ccedil;&atilde;o na prefeitura"></div>
        ...
        <div class="field col-2"><p style="font-size:.8rem;color:var(--text-muted);margin:0">
          ...
          O regime tribut&aacute;rio fica nos Dados da empresa acima.
  ...
  // Certificado NF-e (modo certificado próprio)

  ...
  const crud = new Crud({
    ...
    colunas: [
      { chave: "foto", titulo: "", render: (v, row) => {
          // Produto pai com grade — não mostra coluna de foto
          if (row.tem_variacoes && !row.produto_pai_id) return "";
          return v
            ? `<img src="${v}" style="width:38px;height:38px;object-fit:cover;border-radius:6px;border:1px solid #eee;display:block">`
    ...
    wrap.innerHTML = `
      ...
      <div style="display:flex;gap:.75rem;align-items:center">
        <div id="prod-foto-preview" style="width:80px;height:80px;border-radius:8px;
          ...
          background:#f8f9fa;overflow:hidden;flex-shrink:0">
          ${fotoAtual
            ? `<img src="${fotoAtual}" style="width:100%;height:100%;object-fit:cover">`
